...
def installed_power(cloud_event):
  ...
  if stats is True:
    ...
    rows_to_insert = []
    for i, year in enumerate(data['time']):
      for production_type in data['production_types']:
        rows_to_insert.append((year, production_type['name'], production_type['data'][i]))
